...
from models.han  import HANLayer
...
class Gat(BaseGraphModel):
    def __init__(self, args, dataloader):
        super(Gat, self).__init__(args, dataloader)
        self.W = torch.nn.Parameter(torch.randn(self.args.embed_size, self.args.embed_size))
        self.a = torch.nn.Parameter(torch.randn(self.args.embed_size))

    def build_layer(self, idx):
        return GatLayer(self.args)
...
class HAN(BaseGraphModel):
    def __init__(self, args, dataloader):
        super(HAN, self).__init__(args, dataloader)
        self.W = torch.nn.Parameter(torch.randn(self.args.embed_size, self.args.embed_size))
        self.a = torch.nn.Parameter(torch.randn(self.args.embed_size))

    def build_layer(self, idx):
        return HANLayer(self.args,self.graph)
